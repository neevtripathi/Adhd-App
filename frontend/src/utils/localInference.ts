import type { CptFeatureVector, PredictionResult, LikelihoodBand, ConfidenceTier } from '../types'

const sigmoid = (x: number) => 1 / (1 + Math.exp(-x))

function getBand(p: number): { band: LikelihoodBand; label: string } {
...
function getConfidence(mods: string[]): { tier: ConfidenceTier; label: string } {
...
export function runLocalInference(data: LocalInferenceInput): PredictionResult {
  const mods: string[] = []
  ...
  if (data.cpt) {
    ...
    if (f.hit_rt_sd_t <= 65 && f.commissions_t <= 60 && f.omissions_t <= 60) {
      factors.push('Your reaction-time patterns and accuracy were within a typical range for this task.')
    }
  }
  ...
  if (data.questionnaire) {
    ...
    else factors.push('Your self-report responses indicate moderate attention-related traits.')
  ...
  if (factors.length === 0) factors.push('A baseline estimate was generated from the available data.')

  const prob = Math.max(0.05, Math.min(0.95, sigmoid(logit)))
  const spread = mods.length === 3 ? 0.10 : mods.length === 2 ? 0.15 : 0.22
  const { band, label } = getBand(prob)
  const { tier, label: cLabel } = getConfidence(mods)

  return {
    adhd_probability: Math.round(prob * 100) / 100,
    likelihood_band: band,
    likelihood_band_label: label,
    modalities_used: mods,
    confidence_tier: tier,
    confidence_label: cLabel,
    uncertainty_interval: [
      Math.round(Math.max(0.02, prob - spread) * 100) / 100,
      Math.round(Math.min(0.98, prob + spread) * 100) / 100,
    ],
    branch_contributions: { cpt: cptP, activity: actP, hrv: hrvP },
    contributing_factors: factors,
    data_quality_flags: flags,
    model_version: '1.0.0-client',
